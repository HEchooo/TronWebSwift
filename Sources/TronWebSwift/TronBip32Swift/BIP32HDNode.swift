...
public class HDNode {
    public struct HDversion{
        public var privatePrefix: Data = Data([0x04, 0x88, 0xAD, 0xE4])
        public var publicPrefix: Data = Data([0x04, 0x88, 0xB2, 0x1E])
        public init() {
    ...
    public convenience init?(_ serializedString: String) {
        let data = Data(TronBase58.bytesFromBase58(serializedString))
        self.init(data)
    }
    ...
    public init?(seed: Data) {
        ...
        let hmac: CAuthenticator = CHMAC(key: hmacKey.bytes, variant: .sha2(.sha512))
...
extension HDNode {
        ...
        if derivePrivateKey {
            if self.hasPrivate { // derive private key when is itself extended private key
                ...
                if index >= (UInt32(1) << 31) || hardened {
                    ...
                    let hmac:CAuthenticator = CHMAC(key: self.chaincode.bytes, variant: .sha2(.sha512))
    ...
    public func serializeToString(serializePublic: Bool = true, version: HDversion = HDversion()) -> String? {
        ...
        let encoded = TronBase58.base58FromBytes(data.bytes)
        return encoded
    }
